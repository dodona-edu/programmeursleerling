...
def oorlogje(stapel_1, stapel_2):
    ...
    """
    Iedere ronde trekken beide stapels hun bovenste kaart. De hoogste van de
    twee kaarten gaat onderop de stapel waar hij vandaan kwam, en de andere
    kaart gaat er achteraan. Twee kaarten met dezelfde waarde kunnen niet
    voorkomen, dus er is altijd een winnaar van de ronde.
    ...
    while len(stapel_1) > 0 and len(stapel_2) > 0:

        kaart_1 = stapel_1.trek()
        kaart_2 = stapel_2.trek()

        if kaart_1 > kaart_2:
            stapel_1.voegtoe(kaart_1)
            stapel_1.voegtoe(kaart_2)
        else:
            stapel_2.voegtoe(kaart_2)
            stapel_2.voegtoe(kaart_1)

    return stapel_1 if len(stapel_1) > 0 else stapel_2
